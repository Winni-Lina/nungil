...
public class StepGenerationService {
    ...
    public List<String> generatePersonalizedSteps(String taskProcess, String location,
        ...
        String system = "## 역할\n"
                ...
                + "## 작성 규칙\n"
                + "- 각 단계는 \"~하세요\" 형태의 직접 명령문, 15자 내외로 아주 짧고 쉽게\n"
                + "- 어려운 단어 금지, 한 단계에 한 가지 행동만\n"
                + "- 각 단계는 [무엇을 + 어디에/어떻게] 구조로 작성 (예: \"수건을 세탁기에 넣으세요\")\n"
                + "- 위치·순서·모양처럼 일반적으로 확실한 시각 단서를 포함 (예: \"세탁기 문 옆 칸\", \"둥근 통\")\n"
                + "- 색깔·구체적 위치처럼 집마다 다른 정보는 지어내지 말 것 (그 확인은 사용자가 카메라로 함)\n"
                + "- 장소와 특이사항을 반영해 구체적으로\n"
                + "- 5~10개 단계로\n\n"
